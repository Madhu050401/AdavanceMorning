...
public class webdriverUtility {
	...
	{
		Select sel = new Select(element); sel.selectByIndex(index);
	}
	
	public void select(WebElement element , String value) 
	{
		Select sel = new Select(element); sel.selectByValue(value);
	}
	...
	{
		Actions act = new Actions(driver); act.moveToElement(element).perform();
	}
	...
	{
		Actions act = new Actions(driver); act.doubleClick(element).perform();
	}
	...
	{
		Actions act = new Actions(driver); act.click(element).sendKeys(text).perform();
	}
	...
	{
		TakesScreenshot ts = (TakesScreenshot)driver; File temp = ts.getScreenshotAs(OutputType.FILE); File perm = new
		File("./errorShots"+filename+".png"); org.openqa.selenium.io.FileHandler.copy(temp,
		perm);
